...
class RBM():
	...
	def train(self, data_train, initialmomentum, finalmomentum, learn_rate_w,
		...
		for i in range(self.iterations):
			...
			hidden_state = self._sigmoid(np.dot(data_train, m_weights) \
			...
			if self.visible_type == "SIGMOID":
				reconstruction = self._sigmoid(reconstruction)
	...
	def _CD1(self, visible_data, weights, visible_bias, hidden_bias):
		...
		if self.hidden_type == "SIGMOID":
			hidden_probability = self._sigmoid(nw) 
		...
		if self.visible_type == "SIGMOID":
			visible_state = self._sigmoid(visible_state)
			#visible_probability = self._sigmoid(visible_state)
		...
		nw = np.dot(visible_state, weights) + np.tile(hidden_bias, (N, 1))
		if self.hidden_type == "SIGMOID":
			hidden_probability = hidden_probability = self._sigmoid(nw) 
